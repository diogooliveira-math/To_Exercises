...
```
Read: {project-root}/./bmb/reference/workflows/meal-prep-nutrition/workflow.md
```
...
- **Linear**: Like Meal Prep Plan (Init → Profile → Assessment → Strategy → Shopping → Prep)
  - See: `{project-root}/./bmb/reference/workflows/meal-prep-nutrition/`
...
- For bmb module: Workflows go to `{custom_workflow_location}` (defaults to `./custom/src/workflows`)
