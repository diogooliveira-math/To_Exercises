...
<critical>The workflow execution engine is governed by: {project-root}/./core/tasks/workflow.xml</critical>
<critical>You MUST have already loaded and processed: {project-root}/./bmb/workflows/create-module/workflow.yaml</critical>
<critical>Study existing modules in: {project-root}/./ for patterns</critical>
...
<check if="create">
  <action>Invoke module-brief workflow: {project-root}/./bmb/workflows/module-brief/workflow.yaml</action>
...
2. **Module code** - Generate kebab-case from name following patterns:
   ...
   - Present suggested code and confirm it works for paths like ./{{code}}/agents/
...
# EXAMPLE Static path:
# data_path:
#   result: "{project-root}/./{{module_code}}/data"
...
The module can be configured in `./{{module_code}}/config.yaml`
